...
@property (nonatomic, strong) UIButton *invokeButton;
@end
...
+ (void)initialize {
  if (self == [DemoViewController self]) {
    gModelPaths = @[
...
- (void)viewDidLoad {
  [super viewDidLoad];
    ...
    self.modelControl = [[UISegmentedControl alloc] initWithItems:@[@"one", @"two", @"three"]];
    self.modelControl.frame = CGRectMake(0, 50, 300, 50);
    self.modelControl.tintColor=[UIColor redColor];
    self.modelControl.selectedSegmentIndex=0;
    [self.modelControl addTarget:self action:@selector(changeIndex:) forControlEvents:UIControlEventValueChanged];
    [self.view addSubview:self.modelControl];
    ...
    [self updateResultsText:[NSString stringWithFormat:@"Using TensorFlow Lite runtime version %@.", TFLVersion]];
    [self loadModel];
}
...
- (void)invokeButtonClick:(id)sender {
...
- (UITextView *)resultsTextView {
    if (!_resultsTextView) {
        _resultsTextView = [[UITextView alloc] initWithFrame:CGRectMake(0, 200, 300,400)];
        _resultsTextView.backgroundColor = [UIColor blueColor];
    }
    return _resultsTextView;
}

- (UIButton *)invokeButton {
    if (!_invokeButton) {
        _invokeButton = [UIButton buttonWithType:UIButtonTypeCustom];
        _invokeButton.frame = CGRectMake(150, 150, 60, 30);
        [_invokeButton setTitle:@"invoke" forState:UIControlStateNormal];
        [_invokeButton setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
        [_invokeButton addTarget:self action:@selector(invokeButtonClick:) forControlEvents:UIControlEventTouchUpInside];
    }
    return _invokeButton;
}
@end
